...
function checkBalanced (tree, currDepth = 0) {
  ...
  if (tree.left) { //if there is a left side of the tree we need to recurse down that side
    leftResults = leftResults.concat(checkBalanced(tree.left, currDepth)) //adding the results of the recursive call to our results
    if (!tree.right) rightResults = rightResults.concat([currDepth]) //if the current tree does not have a right side then we will want to compare the current depth to whatever our recursive call to the left returns
  }

  //same logic as above but for the right side of the tree.
  if (tree.right){
    rightResults = rightResults.concat(checkBalanced(tree.right, currDepth))
    if (!tree.left) leftResults = leftResults.concat([currDepth])
  }

  if (!leftResults[0] || !rightResults[0]) return false //if either the left or the right sides of the tree has returned us false than just return false
  if (Math.abs(leftResults[0] - rightResults[0]) > 1) return false //if the difference between the depths from the left side and the right side are greater than 1 that means we have found an imbalance and we will just return false

  const largerDepth = leftResults[0] > rightResults[0] ? leftResults : rightResults //we find the longer of the two depths and return that so that we can use that to compare to our previous call
